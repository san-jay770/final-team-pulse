 ...
 * Full CRUD with email notifications on create, activate, deactivate,
 * password reset and PostgreSQL compatibility.
 */
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ========================================================
      // Check existing user in same team
      // ========================================================
      ...
      await run(
        ...
        [
          req.user.id,
          `${req.user.name} added user ${effectiveName} to team`
        ]
      );
      ...
      if (
        ...
        return res.status(409).json({
          success: false,
          message:
            'User already exists in this team.'
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      return res.json({
        success: true,
        message:
          'Cellular phone updated successfully.',
        phone: phone.trim()
      });

    } catch (err) {
      console.error(
        '[Users] Phone update error:',
        err.message
      );
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      // ADMIN can delete only their team
      if (
        req.user.role === 'ADMIN' &&
        user.team_id !== req.user.team_id
      ) {
      ...
      await run(
        ...
        [
          req.user.id,
          `${req.user.name} deleted user ${user.name} (${user.email})`
        ]
      ).catch(() => { });
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      // Sync password across all teams
      await run(
        `
          UPDATE users
          SET password = ?
      ...
      emailService
        ...
        .catch(err => {
          console.error(
            '[Users] Password reset email error:',
            err.message
          );
        });
      ...
      await run(
        ...
        [
          req.user.id,
          `${req.user.name} reset password for ${user.name}`
        ]
      );
